import React, { Suspense, lazy, useEffect, useState, SetStateAction } from 'react';
...
const ListState = () => {
    ...
    const getCards = (e: any) => <Cards key={e.id} nome={e.nome} sigla={e.sigla} regiao={e.regiao.nome} link={() => e} />
    ...
    useEffect(
        () => {
            (async () => {
                ...
                setRenderData(data.map((info: any) => getCards(info)))
            })()
        }, []
    )
    ...
    const searchAction = (e: any) => {
        ...
        setRenderData(searchCards.map((info) => getCards(info)))
    }

    return (
        <Suspense fallback={<Loading />}>
            <h1 className="py-3" data-testid="title-list-state"> Estados do Brasil </h1>
